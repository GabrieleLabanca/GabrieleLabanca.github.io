...
var x_domain_min = new Date(2004, 12, 1);
...
function make_plot(){
  ...
  var b_space = 10;
  var one_year = new Date(2005,12,1);
  bin_w = x(one_year) - b_space; // a bin is one-year wide
  function get_x(d){
    return d[x_label];
  }
  function get_y(d){
  ...
  main_g.selectAll("rect")
    ...
    .attr("fill","#3B3838")
    .attr("x",function(d){ return x(get_x(d)) + b_space - 0.5*x(one_year);})
    .attr("y",function(d){ return (y(get_y(d))); });

  console.log(main_g.selectAll("rect"));
}
